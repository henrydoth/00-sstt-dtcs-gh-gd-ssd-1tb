...
format: 
  html:
    ...
    toc_float: false  # Ensures TOC does not float
editor: visual
---
...
```{r}
#extract thang kham
#df$month.year <- format(as.Date(df$date.visit), "%Y-%m")
...
## Biểu đồ giới tính
```{r}
ggdonutchart(df %>% select(gender) %>% count(gender) %>% slice(1:2) , "n", label = "gender",
...
```{r}
print(my_pres , "dtcs_sstt_quarto_03_2025.pptx")
```
